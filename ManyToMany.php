...
trait ManyToMany
{
    ...
    {
        $target = basename($target_class);
        $bridge = basename($bridge_class);
        ...
        $this->addMethod($target, function ($m, $data='', $field='') {
            ...
            if (!empty($data)) {
                ...
                return $this->ref(basename($bridge_class))->ref(basename($target_class))->addCondition($field, $data);
            }
            else
            {
                return $this->ref(basename($bridge_class))->ref(basename($target_class));
            }
        });
        ...
        $this->addMethod('add' . $target, function ($m, $data, $field='') {
            ...
            $field = empty($field) ? $default_field : $field;
            if (!$this->loaded()) {
                throw new \Exception('No data loaded for ' . basename($this) .'. ' . basename($target_class) . ' can not be accessed without a dataset loaded.');
            }
            ...
            if (!empty($data)) {
                ...
                try {
                    foreach ($data as $item) {
                        $t = new $target_class($this->persistence);
                        $t->loadBy($field, trim($item));
                        $this->ref(basename($bridge_class))->addCondition($target_their_field, $t[$target_class::our_field])->each(function ($m) {
                            $m->delete();
                        });
                    }
                } catch (\atk4\data\Exception $e) {
        ...
        $this->addMethod('has' . $target, function ($m, $data='', $field='') {
            ...
            if (!empty($data)) {
                $data = is_array($data) ? $data : explode(',', $data);
                return $this->ref(basename($bridge_class))->ref(basename($target_class))->addCondition($field, $data)->action('count')->getOne();
            }
            else
            {
                return $this->ref(basename($bridge_class))->ref(basename($target_class))->action('count')->getOne();
            }
        });
    }
    ...
    public function addBridgeBetween($a, $b)
    {
        ...
        $this->hasOne(basename($a), [$a, 'our_field'=> $a_their_field, 'their_field'=>$a::our_field]);
        $this->hasOne(basename($b), [$b, 'our_field'=> $b_their_field, 'their_field'=>$b::our_field]);
    }
